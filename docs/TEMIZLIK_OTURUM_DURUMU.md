...
- 🔶 **Adım 4d — routers/ (DEVAM EDİYOR).** Kanıtlanmış yöntem + güvenlik ağı kuruldu.
  - **Güvenlik ağı (commit'li):** `tests/test_route_inventory.py` (578 route donduruldu — kaybolan/değişen route'u yakalar) + `tests/test_smoke_admin_get.py` (~245 admin GET → 500 yok; SSE atlanır, her istek 8s sınırlı) + 442 davranış testi. Toplam **445 test**.
  - **Çıkarılanlar (commit'li):** `ratelimit.py` (limiter — #1 blocker), sonra route domain'leri: `raffles_api`, `event_sponsors_api`, `certificate_tiers_api`, `survey_config_api`, `template_history_api`, `ms_excel_api`, `bulk_generate_api`. main.py 15909 → **14789**.
  - **KİLİT İÇGÖRÜ:** Yeni router `from .main import <helper'lar>` yapar; main bu router'ı EN SONDA include ettiği için (mevcut 43 router gibi) geç-import çalışır → helper'ları önce çıkarmaya GEREK YOK. `@app.X` → `@router.X`, sona `app.include_router`.
  - **Extractor reçetesi (öğrenilen fix'ler):** AST ile keyword'e uyan `@app` handler'larını bul (contiguous olmalı, değilse atla). `from_main` = bloktaki Load isimleri − builtins − **fonksiyon parametreleri** − **`except ... as <ad>` binding'leri** (AST'de string!) − handler adları − standart header isimleri. Router header'ı MUTLAKA tam `typing` + tam `stdlib` (secrets/re/hashlib/hmac/base64/asyncio/...) + fastapi + sqlalchemy import etmeli (yoksa NameError).
  - **KALAN (~190 handler):** Çoğu **interleaved** (kaynak kodda iç içe): registration/sessions/attendees/support-tickets (11758-12842 arası karışık), badges/tiers, api-keys. Bunlar contiguous-blok ile çıkmaz → handler-bazlı taşıma veya bölgeyi domain'lere ayırma gerekir. Temiz contiguous domain'ler azaldı.
