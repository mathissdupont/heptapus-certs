...
HeptaCert komut satırı aracı — otomasyon, scripting ve hızlı yönetim için. REST API'nin tamamını terminalden kullanılabilir kılar ve aynı [API anahtarı + scope](/api-reference/authentication) modelini paylaşır.
...
Webhook olayları ve imza doğrulama için [Webhooks](/integrations/webhooks) sayfasına bakın.
...
- [Aynı işlemler AI asistanı ile (MCP) →](/integrations/mcp-agent)
