  ...
  return (
    <section className="w-full rounded-2xl border border-surface-200/80 bg-white p-5 sm:p-6 shadow-sm antialiased">
      ...
      {loading ? (
        ...
        <div className="relative pl-4 sm:pl-5 before:absolute before:bottom-2 before:left-1.5 before:top-2 before:w-[1px] before:bg-surface-100">
              ...
              <div key={item.id} className="relative group">
                ...
                <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <h3 className="text-xs font-semibold text-surface-900 tracking-tight">
                      {item.action_label}
                    </h3>
                    {item.detail && (
                      <p className="text-xs leading-relaxed text-surface-500 max-w-2xl">
                        {item.detail}
                      </p>
                    )}
                    <p className="text-11 font-medium text-surface-400 pt-0.5">
                      {item.actor_label}
                    </p>
                  </div>
