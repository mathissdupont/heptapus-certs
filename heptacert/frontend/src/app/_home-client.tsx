...
export default function LandingPage() {
  ...
  const isWhiteLabel = useMemo(() => {
    if (!branding?.org_name) return false;
    if (branding?.settings?.hide_heptacert_home) return true;
  ...
  const heroTitle = isWhiteLabel
    ...
    : `${t("home_hero_title_1").replace("HeptaCert", brandName)} ${t("home_hero_title_2")} ${t("home_hero_title_3")}`;

  const heroSubtitle = isWhiteLabel
    ? `${brandName} için sertifika oluşturma, dağıtma ve doğrulama süreçlerini tek noktadan yönetin.`
    : t("home_hero_subtitle");

  const features = [
    {
      icon: QrCode,
      title: isWhiteLabel ? "QR ile Doğrulama" : t("feat_qr_title"),
      desc: isWhiteLabel
        ? "Sertifikaları QR kod ile hızlı ve güvenli şekilde doğrulayın."
        : t("feat_qr_desc"),
      color: "brand-text brand-bg-50",
    },
    {
      icon: FileCheck,
      title: isWhiteLabel ? "Toplu Sertifika İşleme" : t("feat_excel_title"),
      desc: isWhiteLabel
        ? "Katılımcı listeleriyle sertifika üretim sürecini kolaylaştırın."
        : t("feat_excel_desc"),
      color: "text-emerald-600 bg-emerald-50",
    },
    {
      icon: Search,
      title: isWhiteLabel ? "Kayıt ve Takip" : t("feat_records_title"),
      desc: isWhiteLabel
        ? "Sertifika kayıtlarını ve doğrulama geçmişini takip edin."
        : t("feat_records_desc"),
      color: "text-violet-600 bg-violet-50",
    },
    {
      icon: Users,
      title: isWhiteLabel ? "Etkinlik Yönetimi" : t("feat_events_title"),
      desc: isWhiteLabel
        ? "Katılımcı, oturum ve check-in akışlarını tek panelden yönetin."
        : t("feat_events_desc"),
      color: "text-amber-600 bg-amber-50",
    },
    {
      icon: Lock,
      title: isWhiteLabel ? "Güvenli Altyapı" : t("feat_security_title"),
      desc: isWhiteLabel
        ? "Doğrulama ve kayıt işlemleri güvenli altyapı üzerinde çalışır."
        : t("feat_security_desc"),
      color: "text-rose-600 bg-rose-50",
    },
    {
      icon: BarChart3,
      title: isWhiteLabel ? "Operasyonel Görünürlük" : t("feat_hc_title"),
      desc: isWhiteLabel
        ? "Etkinlik ve sertifika süreçlerinizi ölçülebilir hale getirin."
        : t("feat_hc_desc"),
  ...
  const statCards = [
    { key: "active_orgs", label: isWhiteLabel ? "Aktif Kurumlar" : t("stats_orgs") },
    { key: "certs_issued", label: isWhiteLabel ? "Oluşturulan Sertifikalar" : t("stats_certs") },
    { key: "uptime_pct", label: isWhiteLabel ? "Erişilebilirlik" : t("stats_uptime") },
    { key: "availability", label: isWhiteLabel ? "Durum" : t("stats_availability") },
  ] as const;

  const steps = isWhiteLabel
    ? [
        {
          step: "1",
          title: "Katılımcıları yönetin",
          desc: "Etkinlik ve katılımcı kayıtlarını tek panelden organize edin.",
        },
        {
          step: "2",
          title: "Sertifikaları oluşturun",
          desc: "Şablonlar ve otomasyon ile sertifika süreçlerini hızlandırın.",
        },
        {
          step: "3",
          title: "Doğrulama sağlayın",
          desc: "QR ve UUID ile doğrulama akışını kullanıcılarınıza sunun.",
        },
      ]
    : [
  ...
  return (
    <div className="flex flex-col gap-28 pb-20 pt-4">
      <motion.section variants={stagger} initial="hidden" animate="show" className="text-center pt-10">
        <motion.h1
          variants={in_view}
          className="mx-auto max-w-3xl text-5xl font-extrabold tracking-tight text-gray-900 sm:text-6xl leading-tight"
        >
          {isWhiteLabel ? (
            heroTitle
          ) : (
            <>
              {t("home_hero_title_1").replace("HeptaCert", brandName)}{" "}
        ...
        <motion.div variants={in_view} className="mt-10 flex flex-wrap justify-center gap-4">
          {showStartFreeCta ? (
            ...
            <Link href="/verify" className="btn-primary text-base px-7 py-3.5 group">
              Sertifika Doğrula
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </Link>
          )}
          ...
          <Link href="/verify" className="btn-secondary text-base px-7 py-3.5">
            {isWhiteLabel ? "Doğrulama Sayfası" : t("home_cta_verify")}
          </Link>
        </motion.div>

        <motion.div variants={in_view} className="mt-6 text-sm text-gray-400">
          {isWhiteLabel ? `${brandName} için özelleştirilmiş dijital sertifika deneyimi` : t("home_hero_sub")}
        </motion.div>
      </motion.section>
      ...
      <section id="features" className="scroll-mt-24">
        ...
        >
          <h2 className="text-3xl font-bold text-gray-900">
            {isWhiteLabel ? `${brandName} için öne çıkan yetenekler` : t("home_features_title")}
          </h2>
          <p className="mt-3 text-gray-500 max-w-xl mx-auto">
            {isWhiteLabel
              ? "Kurumsal sertifika ve doğrulama süreçleri için tasarlanmış temel özellikler."
              : t("home_features_sub")}
          </p>
        </motion.div>
        ...
        >
          {features.map((f) => (
            <motion.div key={f.title} variants={in_view} className="card p-6 group hover:shadow-lifted transition-shadow">
      ...
      >
        <h2 className="text-2xl font-bold text-gray-900 mb-10 text-center">
          {isWhiteLabel ? "Nasıl çalışır?" : t("home_how_title")}
        </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {steps.map((item) => (
            <div key={item.step} className="flex flex-col items-center text-center">
              <div
                className="flex h-12 w-12 items-center justify-center rounded-full text-white font-bold text-lg shadow-brand mb-4"
                style={{ backgroundColor: "var(--site-brand-color)" }}
              >
                {item.step}
              </div>
      ...
      {showMarketingBlocks && (
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="max-w-md mx-auto mb-8 text-base" style={{ color: "rgba(255,255,255,0.95)" }}>
              {t("home_cta_section_sub")}
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link
                href="/register"
                className="inline-flex items-center gap-2 rounded-xl bg-white px-7 py-3.5 text-sm font-semibold shadow-lifted transition-colors"
                style={{ color: "var(--site-brand-color)" }}
              >
                {t("home_cta_section_btn1")} <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                href="/pricing"
                className="inline-flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-7 py-3.5 text-sm font-semibold text-white hover:bg-white/20 transition-colors"
              >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-xs font-medium text-gray-400 mt-1">
                <a href="https://heptapusgroup.com/privacy" target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition-colors">
                  {t("heptapus_privacy")}
                </a>
                <a href="https://heptapusgroup.com/terms" target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition-colors">
                  {t("heptapus_terms")}
                </a>
                <a href="mailto:contact@heptapusgroup.com" className="hover:text-brand-600 transition-colors">
                  {t("heptapus_contact_label")}: contact@heptapusgroup.com
                </a>
              </div>
            </div>
      ...
      <footer className="flex flex-col gap-8 border-t border-gray-100 pt-10 pb-6">
        <div className="flex flex-wrap gap-x-6 gap-y-2 justify-center text-xs text-gray-400">
          <Link href="/kvkk" className="hover:text-brand-600 transition-colors">
        ...
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400 font-normal">© {new Date().getFullYear()} {brandName}</span>
          </div>
