    ...
    <div className="space-y-8 pb-12">
      ...
      <section className="card p-6 sm:p-8">
        ...
        <div className="mt-6 space-y-4">
          {member ? (
            <form onSubmit={handleCommentSubmit} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
              <textarea
                value={commentBody}
                onChange={(eventArg) => setCommentBody(eventArg.target.value)}
                rows={4}
                placeholder={copy.commentPlaceholder}
                className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-brand-300 focus:ring-2 focus:ring-brand-100"
              />
              <div className="mt-3 flex justify-end">
                <button type="submit" disabled={commentBusy || !commentBody.trim()} className="btn-primary inline-flex items-center gap-2 disabled:opacity-60">
            ...
            <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-5 py-6 text-sm text-slate-500">
              {copy.loginPrompt}
              <div className="mt-4">
                <Link href="/login?mode=member" className="btn-secondary inline-flex">
                  {copy.loginCta}
                </Link>
              </div>
            </div>
              ...
              <article key={comment.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  {member && member.id !== comment.member_id ? (
                    ...
                    >
                      {reportingId === comment.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
