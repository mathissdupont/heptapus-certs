...
function NavSidebar({ pathname, lang }: { pathname: string; lang: string }) {
  return (
...
export default function SuperadminLayout({
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex items-center gap-3 rounded-2xl border border-amber-200/60 bg-gradient-to-r from-amber-50 to-orange-50 px-5 py-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100 text-amber-700 shadow-sm">
          <Crown className="h-5 w-5" />
        </div>
        <div>
          <h1 className="text-base font-black tracking-tight text-surface-900">
            Super Admin
          </h1>
        ...
        <div className="ml-auto flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-100/60 px-2.5 py-1">
          ...
          <span className="text-[10px] font-bold text-amber-700 uppercase tracking-wide">superadmin</span>
        </div>
      </div>

      {/* 2-column layout */}
      <div className="flex gap-6 items-start">
        <NavSidebar pathname={pathname || ""} lang={lang} />
