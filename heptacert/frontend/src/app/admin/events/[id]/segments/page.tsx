...
export default function EventSegmentsPage() {
  ...
  async function loadSegments() {
    ...
    try {
      ...
      setError(ex?.message || "Segmentler yüklenemedi.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadPreview(key: AudienceSegmentKey) {
    ...
    try {
      ...
      setError(ex?.message || "Segment önizlemesi yüklenemedi.");
    } finally {
  ...
  async function exportSegment(key: AudienceSegmentKey) {
    ...
    if (!response.ok) {
      setError("Segment export alınamadı.");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-black text-surface-900">Katılımcı Segmentleri</h1>
          <p className="mt-2 max-w-2xl text-sm text-surface-500">
            Katılım, sertifika, anket, tekrar katılım ve kayıt cevabı filtreleriyle hedef kitleleri hızlıca ayırın.
          </p>
        </div>
      </div>
      ...
      <section className="surface-panel p-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-black text-surface-900">Dinamik filtre</h2>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto]">
          <input value={fieldId} onChange={event => setFieldId(event.target.value)} className="input-field" placeholder="Kayıt alanı ID, örn. department" />
          <input value={answer} onChange={event => setAnswer(event.target.value)} className="input-field" placeholder="Cevap içerir, örn. Pazarlama" />
          <input value={location} onChange={event => setLocation(event.target.value)} className="input-field" placeholder="Lokasyon/şehir, örn. İstanbul" />
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_420px]">
          ...
          <aside className="surface-panel p-5">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-base font-black text-surface-900">Önizleme</h2>
              {preview && <span className="rounded-full bg-surface-100 px-3 py-1 text-xs font-bold text-surface-500">{preview.segment.count} kişi</span>}
            </div>
            {previewLoading ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="mt-3 text-sm font-bold text-surface-700">Bir segment seçin</p>
                <p className="mt-1 text-xs text-surface-500">İlk 50 katılımcı burada listelenir.</p>
              </div>
            ) : (
                ...
                {preview.attendees.map((attendee) => (
                  <div key={attendee.id} className="rounded-lg border border-surface-200 bg-white px-3 py-2">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {attendee.email_verified && <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700">E-posta onaylı</span>}
                      {attendee.survey_completed && <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700">Anket tamam</span>}
                    </div>
                  </div>
                ))}
              </div>
