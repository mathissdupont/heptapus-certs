...
const ENDPOINTS: { group: string; items: Endpoint[] }[] = [
  ...
  },
  {
    ...
    items: [
      { method: "GET",    path: "/api/public/marketplace",                description: "Listelenmiş programları getir",     scope: undefined },
      { method: "GET",    path: "/api/public/marketplace/{id}",           description: "Program detayı",                    scope: undefined },
    ],
  },
  {
    group: "Formlar (Public)",
    items: [
      { method: "GET",    path: "/api/public/forms/{slug}/meta",          description: "Form tanımını getir",               scope: undefined },
      { method: "POST",   path: "/api/public/forms/{slug}/submit",        description: "Form gönderimi",                    scope: undefined },
    ],
  },
];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold mb-4">Kimlik Doğrulama</h2>
  ...
  -H "Authorization: Bearer hc_YOUR_API_KEY"`}
        ...
        <section className="mb-10">
          ...
          <p className="text-xs text-gray-400 mt-2">
            Rate limit aşıldığında <code>429 Too Many Requests</code> döner.
            <code>Retry-After</code> header'ı bekleme süresini belirtir.
          </p>
        </section>

        {/* Endpoints */}
        <section className="mb-10">
          <h2 className="text-xl font-bold mb-4">Endpoint'ler</h2>
                      ...
                      {group.items.map((ep, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-4 py-3 w-20">
                            <span
                              className={`inline-block px-2 py-0.5 rounded text-xs font-bold font-mono ${METHOD_COLORS[ep.method]}`}
                            >
                              {ep.method}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-mono text-xs text-gray-800 min-w-64">
                            {ep.path}
                          </td>
        ...
        {/* Example */}
        <section className="mb-10">
          <h2 className="text-xl font-bold mb-4">Örnek: Sertifika Listele</h2>
          <div className="bg-white rounded-xl border p-5">
            <p className="text-sm text-gray-600 mb-3">
              Bir etkinliğin sertifika listesini çekin:
            </p>
            <pre className="bg-gray-900 text-green-400 text-xs p-4 rounded-lg overflow-x-auto font-mono">
    ...
    "cert_url": "https://heptacert.com/c/abc123"
        ...
        <div className="bg-indigo-50 rounded-xl border border-indigo-100 p-5 text-center">
          <p className="text-sm text-indigo-800">
            Sorunuz mu var?{" "}
            <a href="mailto:destek@heptacert.com" className="font-medium underline">
