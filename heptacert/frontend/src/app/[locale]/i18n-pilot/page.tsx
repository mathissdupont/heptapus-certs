...
export default async function I18nPilotPage({
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-20">
      <p className="mb-2 text-xs font-bold uppercase tracking-widest text-surface-400">{locale}</p>
      <h1 className="mb-4 text-3xl font-extrabold text-surface-900">{t("home_features_title_default")}</h1>
      <p className="mb-8 text-surface-600">{t("home_features_body")}</p>
      <a
        href={`/${locale}/pricing`}
        className="inline-flex rounded-lg bg-surface-900 px-5 py-2.5 font-bold text-white"
      >
        {t("home_primary_cta")}
      </a>
    </main>
